...
if (-not (Test-Path -LiteralPath $backup)) {
    ...
    Write-Host "Existing prepatch backup retained: $backup"
}
...
function Replace-RegexOnce([string]$Text, [string]$Pattern, [string]$Replacement, [string]$Label) {
    ...
    if ($matches.Count -ne 1) { throw "${Label}: expected exactly one regex match, found $($matches.Count)" }
...
# Helper and Present anchors are deliberately narrow; these parts are unchanged by the
# already-proven source tap. The Normalize function, however, may contain local probe edits,
# so insert the render override relative to the whole function boundary instead of assuming
# an exact tail body.
...
$normalizePattern = '(  void D3D9SwapChainEx::NormalizePresentParameters\(D3DPRESENT_PARAMETERS\* pPresentParams\) \{.*?)(\r?\n  \}\r?\n\r?\n  void D3D9SwapChainEx::PresentImage)'
...
#endif$2
'@
$text = Replace-RegexOnce $text $normalizePattern $normalizeReplacement 'NormalizePresentParameters render override'

[IO.File]::WriteAllText($source, $text, (New-Object Text.UTF8Encoding($false)))

$verify = [IO.File]::ReadAllText($source)
if (-not $verify.Contains('[M3K-S1B] config') -or
    -not $verify.Contains('M3kApplyRenderOverride(pPresentParams);') -or
    -not $verify.Contains('M3kScheduleAutoResize(window, m_presentParams.Windowed);')) {
