[CmdletBinding()]
param()
$ErrorActionPreference = 'Stop'

$toolRoot = $PSScriptRoot
$workRoot = Join-Path $toolRoot '_work'
$baseOut = Join-Path $toolRoot 'out-m3k'
$out = Join-Path $toolRoot 'out-m3k-s22'
$generated = Join-Path $workRoot 'm3k-src-s22'
$obj = Join-Path $workRoot 'obj-s22'

function Run([string]$Program, [string[]]$Arguments) {
    & $Program @Arguments
    if ($LASTEXITCODE -ne 0) { throw "$Program failed with exit code $LASTEXITCODE" }
}

Write-Host '=== A2-S2.2 gate 1/4: reproduce proven A2-S1 dependencies/build ==='
& (Join-Path $toolRoot 'build.ps1')
if (-not (Test-Path -LiteralPath (Join-Path $baseOut 'BUILD-VERIFIED.txt'))) {
    throw 'Baseline A2-S1 build verification marker missing'
}

$feeder = Join-Path $workRoot 'feeder'
$ngx = Join-Path $workRoot 'ngx-sdk'
$vulkan = Join-Path $workRoot 'vulkan-headers'
foreach ($p in @($feeder, $ngx, $vulkan)) {
    if (-not (Test-Path -LiteralPath $p)) { throw "Missing prepared dependency: $p" }
}

Write-Host '=== A2-S2.2 gate 2/4: generate proven low-res NR -> SR source ==='
$stageS2 = Join-Path $toolRoot 'a2-s2-stage.ps1'
if (-not (Test-Path -LiteralPath $stageS2)) { throw "Missing A2-S2 stage: $stageS2" }
& $stageS2 -SourceRoot (Join-Path $toolRoot 'src') -GeneratedRoot $generated

Write-Host '=== A2-S2.2 gate 3/4: add independent multi-pass NR chain ==='
$stageMp = Join-Path $toolRoot 'a2-s22-multipass-stage.ps1'
if (-not (Test-Path -LiteralPath $stageMp)) { throw "Missing A2-S2.2 stage: $stageMp" }
& $stageMp -GeneratedRoot $generated

$vkText = [IO.File]::ReadAllText((Join-Path $generated 'm3k_vk.h'))
$nrText = [IO.File]::ReadAllText((Join-Path $generated 'm3k_nr.h'))
foreach ($marker in @(
    'M3K-A2-S2: feature 18 armed at true source',
    'static constexpr unsigned MaxPasses = 3;',
    'independent feature 18 chain creation SUCCESS',
    'M3K-A2-S2.2: NRPasses=%u',
    'M3K-A2-S2.2: %s passes=%u -> DLSS SR running')) {
    if (($vkText + $nrText).IndexOf($marker, [StringComparison]::Ordinal) -lt 0) {
        throw "Generated source missing marker: $marker"
    }
}

if (Test-Path -LiteralPath $out) { Remove-Item -LiteralPath $out -Recurse -Force }
if (Test-Path -LiteralPath $obj) { Remove-Item -LiteralPath $obj -Recurse -Force }
New-Item -ItemType Directory -Force -Path $out, (Join-Path $out 'm3k'), $obj, (Join-Path $out 'licenses') | Out-Null

Write-Host '=== A2-S2.2 gate 4/4: compile generated source + CPU tests ==='
Run (Join-Path $toolRoot 'compile-s2.bat') @($feeder, $ngx, $vulkan, $out, $obj, $generated)
Run (Join-Path $out 'm3k-tests.exe') @((Join-Path $out 'm3k\m3k-nvngx.dll'))

Copy-Item -LiteralPath (Join-Path $toolRoot 'm3k-nr.ini') -Destination $out
Copy-Item -LiteralPath (Join-Path $toolRoot 'README.md'), (Join-Path $toolRoot 'NOTICE') -Destination $out
Copy-Item -Path (Join-Path $baseOut 'licenses\*') -Destination (Join-Path $out 'licenses') -Force

$hashes = Get-FileHash -Algorithm SHA256 -LiteralPath `
    (Join-Path $out 'dlss5-feed.addon64'), `
    (Join-Path $out 'm3k\m3k-nvngx.dll')

@(
    'M3K A2-S2.2 x64 build passed compile and CPU-only contract/fallback/shim tests.',
    'Functional target: true low-res DXVK source -> 1/2/3 independent feature18 NR passes -> DLSS SR -> presenter.',
    'Each NR pass has a separate feature handle, NGX parameter block, output texture, and temporal history.',
    'NRPasses=1 preserves the proven single-pass architecture. NRPasses=2 is the first GPU test target.',
    'Jitter remains 0; temporal-quality work is still a later milestone.',
    'This is NOT yet a GTA multi-pass GPU runtime validation.',
    'Feeder=3f624855276c4bde55145c712782477639b30e85',
    'NGX SDK=374959484e79a640feaba44c93ac8cfb0a03f5b5',
    'Vulkan headers=2cd90f9d20df57eac214c148f3aed885372ddcfe',
    ($hashes | ForEach-Object { "$($_.Hash)  $([IO.Path]::GetFileName($_.Path))" })
) | Set-Content -Encoding UTF8 -LiteralPath (Join-Path $out 'BUILD-VERIFIED.txt')

Write-Host "M3K A2-S2.2 BUILD VERIFIED: $out (manual install only)"
