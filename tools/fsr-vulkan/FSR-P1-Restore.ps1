...
if(-not(Test-Path -LiteralPath $State)){throw "No saved FSR P1 state exists at $State"}

# Preserve the test evidence before restoring the user's old log.
if(Test-Path -LiteralPath $Log){
    Copy-Item -LiteralPath $Log -Destination (Join-Path $PackageDir 'FSR-P1-TEST-LOG.txt') -Force
}
...
$pre=(Get-Content -LiteralPath (Join-Path $State 'STATE.txt') -ErrorAction SilentlyContinue)
Remove-Item -LiteralPath $State -Recurse -Force

Write-Host ''
Write-Host 'PRE-TEST STATE RESTORED.'
Write-Host 'The exact previous feeder and whole m3k-nr.ini are back.'
if(Test-Path -LiteralPath (Join-Path $PackageDir 'FSR-P1-TEST-LOG.txt')){
    Write-Host 'Test log saved here:'
    Write-Host "  $(Join-Path $PackageDir 'FSR-P1-TEST-LOG.txt')"
