...
$Candidate=Join-Path $PackageDir 'dlss5-feed-fsr-p4a.addon64'
...
foreach($p in @($Live,$Ini)){if(-not(Test-Path -LiteralPath $p)){throw "Current install file missing: $p"}}
if(Test-Path -LiteralPath $State){throw "A saved FSR P4A state already exists at $State. Run 2-RESTORE-BEFORE-FSR-P4A.bat first; refusing to overwrite it."}
...
try{
    ...
    Write-Host ''
    Write-Host 'FSR P4A EXACT-RASTER TEST APPLIED.'
    Write-Host 'Your production d3d9.dll / b-bridge was NOT changed.'
    Write-Host ''
    Write-Host 'Launch GTA IV, reach gameplay, slowly pan, fast-pan, then drive for about 60 seconds.'
    Write-Host 'Close GTA IV and run 2-RESTORE-BEFORE-FSR-P4A.bat.'
}
catch{
    Write-Host ''
