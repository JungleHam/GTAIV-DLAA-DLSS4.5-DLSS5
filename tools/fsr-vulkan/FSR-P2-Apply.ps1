...
$Candidate=Join-Path $PackageDir 'dlss5-feed-fsr-p2.addon64'
...
if(Test-Path -LiteralPath $State){throw "A saved FSR P2 state already exists at $State. Run 2-RESTORE-BEFORE-FSR-P2.bat first; refusing to overwrite the clean backup."}
...
try {
    ...
    Write-Host ''
    Write-Host 'FSR P2 TEST APPLIED.'
    Write-Host 'P2 target: FSR owns the render-size plan; DLSS SR is disabled.'
    Write-Host 'Expected 1440p source plan is about 1708x960 at FSRScalePermille=667.'
    Write-Host ''
    Write-Host 'Launch GTA IV, reach gameplay, move camera slowly/quickly and drive for about a minute.'
    Write-Host 'Then CLOSE GTA IV and run 2-RESTORE-BEFORE-FSR-P2.bat.'
}
catch {
    Write-Host ''
