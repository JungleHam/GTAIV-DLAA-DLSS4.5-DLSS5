...
if(-not(Test-Path -LiteralPath $State)){throw "No saved FSR P3 state exists at $State"}

if(Test-Path -LiteralPath $Log){Copy-Item -LiteralPath $Log -Destination (Join-Path $PackageDir 'FSR-P3-TEST-LOG.txt') -Force}
...
Write-Host ''
Write-Host 'PRE-P3 STATE RESTORED.'
Write-Host 'The exact previous feeder, INI and old log are back.'
if(Test-Path -LiteralPath (Join-Path $PackageDir 'FSR-P3-TEST-LOG.txt')){
    Write-Host 'P3 test log saved here:'
    Write-Host "  $(Join-Path $PackageDir 'FSR-P3-TEST-LOG.txt')"
