...
$Candidate=Join-Path $PackageDir 'dlss5-feed-fsr-p9c4.addon64'
...
$State=Join-Path $Trex '_FSR_P9C4_ALL_BACKENDS_TEST_STATE'
if(-not(Test-Path $Candidate)){throw "P9C4 candidate missing: $Candidate"}
foreach($p in @($Live,$Ini)){if(-not(Test-Path $p)){throw "Current install file missing: $p"}}
if(Test-Path $State){throw "A saved P9C4 state already exists at $State. Run 2-RESTORE-BEFORE-P9C4.bat first."}
...
try{
    ...
    Set-Ini $Ini 'M3K' 'NvidiaJitterCompMode' '0'
    ...
    Write-Host ''
    Write-Host 'P9C.4 NVIDIA FULL-AA JITTER-SIGN TEST APPLIED.'
    ...
    Write-Host 'This test preserves P9C.2 fixed-exposure FSR and full raster jitter. Only NVIDIA NGX X/Y jitter compensation can be changed live.'
    Write-Host 'Focus: Off -> NVIDIA Quality. Open Advanced -> Temporal AA and cycle the four NVIDIA Jitter Compensation signs.'
