...
# DLAA presenter baseline remains available internally but is intentionally hidden from
# the public dropdown.
$qualityStartMarker = '        ImGui::TextUnformatted("DLSS reconstruction (live)");'
$qualityEndMarker = '        const UINT requested = M3kRequestedNrPasses();'

$qualityStart = $feed.IndexOf($qualityStartMarker, [StringComparison]::Ordinal)
if ($qualityStart -lt 0) {
    throw 'Public ReShade controls: DLSS quality block start marker not found'
}

$qualityEnd = $feed.IndexOf(
    $qualityEndMarker,
    $qualityStart + $qualityStartMarker.Length,
    [StringComparison]::Ordinal
)
if ($qualityEnd -lt 0 -or $qualityEnd -le $qualityStart) {
    throw 'Public ReShade controls: DLSS quality block end marker not found'
}

$qualitySecond = $feed.IndexOf(
    $qualityStartMarker,
    $qualityStart + $qualityStartMarker.Length,
    [StringComparison]::Ordinal
)
if ($qualitySecond -ge 0) {
    throw 'Public ReShade controls: multiple DLSS quality block start markers found'
}
