...
$Candidate=Join-Path $PackageDir 'dlss5-feed-fsr-p9b1.addon64'
...
$State=Join-Path $Trex '_FSR_P9B1_LIVE_NVIDIA_OFF_TEST_STATE'
if(-not(Test-Path $Candidate)){throw "P9B1 candidate missing: $Candidate"}
foreach($p in @($Live,$Ini)){if(-not(Test-Path $p)){throw "Current install file missing: $p"}}
if(Test-Path $State){throw "A saved P9B1 state already exists at $State. Run 2-RESTORE-BEFORE-P9B1.bat first."}
...
try{
    ...
    Write-Host ''
    Write-Host 'P9B.1 LIVE OFF <-> NVIDIA TEST APPLIED.'
    Write-Host 'Initial backend: Off (sessionless raw/native). NVIDIA restores Quality, NR off.'
    ...
    Write-Host 'Switch one step at a time and wait for the transition status to disappear.'
    Write-Host 'Test ONLY: startup Off -> NVIDIA -> Off -> NVIDIA -> Off. Do NOT select AMD in P9B.1.'
