...
if(-not(Test-Path $State)){throw "No saved FSR P6 state exists at $State"}

if(Test-Path $Log){Copy-Item $Log (Join-Path $PackageDir 'FSR-P6-TEST-LOG.txt') -Force}
...
Write-Host ''
Write-Host 'PRE-P6 STATE RESTORED.'
if(Test-Path (Join-Path $PackageDir 'FSR-P6-TEST-LOG.txt')){
    Write-Host 'Send me FSR-P6-TEST-LOG.txt from this folder.'
}
