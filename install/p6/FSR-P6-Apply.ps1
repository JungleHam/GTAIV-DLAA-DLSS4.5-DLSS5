...
$Candidate=Join-Path $PackageDir 'dlss5-feed-fsr-p6.addon64'
...
$State=Join-Path $Trex '_FSR_P6_CAMERA_TEST_STATE'
if(-not(Test-Path $Candidate)){throw "Test candidate missing: $Candidate"}
foreach($p in @($Live,$Ini)){if(-not(Test-Path $p)){throw "Current install file missing: $p"}}
if(Test-Path $State){throw "A saved FSR P6 state already exists at $State. Run 2-RESTORE-BEFORE-FSR-P6.bat first."}
...
try{
    ...
    Write-Host ''
    Write-Host 'FSR P6 CALIBRATED CAMERA TEST APPLIED.'
    Write-Host 'd3d9.dll and NvRemixBridge.exe were NOT changed.'
    Write-Host 'Camera contract: near=0.05 far=1500 vertical-FOV=45deg normal finite depth.'
    Write-Host ''
    Write-Host 'Launch GTA IV, verify menu/HUD, then drive and pan for 30-60 seconds.'
    Write-Host 'Close GTA IV and run 2-RESTORE-BEFORE-FSR-P6.bat.'
}catch{
    try{
