...
$Candidate=Join-Path $PackageDir 'dlss5-feed-fsr-p9.addon64'
...
$State=Join-Path $Trex '_FSR_P9_LIVE_SWITCH_TEST_STATE'
if(-not(Test-Path $Candidate)){throw "P9 candidate missing: $Candidate"}
foreach($p in @($Live,$Ini)){if(-not(Test-Path $p)){throw "Current install file missing: $p"}}
if(Test-Path $State){throw "A saved P9 state already exists at $State. Run 2-RESTORE-BEFORE-P9.bat first."}
...
try{
    ...
    Write-Host ''
    Write-Host 'P9 LIVE BACKEND SWITCH TEST APPLIED.'
    Write-Host 'Initial backend: AMD - FSR, 66.7%, RCAS 0.25.'
    ...
    Write-Host 'Switch one step at a time and wait for the yellow switching line to disappear.'
    Write-Host 'Suggested sequence: AMD -> Off -> AMD -> NVIDIA -> Off -> NVIDIA -> AMD.'
