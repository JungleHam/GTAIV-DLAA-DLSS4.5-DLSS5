...
if(-not(Test-Path $State)){throw "No saved P9A state exists at $State"}
if(Test-Path $Log){Copy-Item $Log (Join-Path $PackageDir 'FSR-P9A-TEST-LOG.txt') -Force}
...
Write-Host ''
Write-Host 'PRE-P9A STATE RESTORED.'
if(Test-Path (Join-Path $PackageDir 'FSR-P9A-TEST-LOG.txt')){Write-Host 'Send me FSR-P9A-TEST-LOG.txt.'}
