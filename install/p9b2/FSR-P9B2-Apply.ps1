...
$Candidate=Join-Path $PackageDir 'dlss5-feed-fsr-p9b2.addon64'
...
$State=Join-Path $Trex '_FSR_P9B2_LIVE_NVIDIA_OFF_TEST_STATE'
if(-not(Test-Path $Candidate)){throw "P9B2 candidate missing: $Candidate"}
foreach($p in @($Live,$Ini)){if(-not(Test-Path $p)){throw "Current install file missing: $p"}}
if(Test-Path $State){throw "A saved P9B2 state already exists at $State. Run 2-RESTORE-BEFORE-P9B2.bat first."}
...
try{
    ...
    Write-Host ''
    Write-Host 'P9B.2 LIVE OFF <-> NVIDIA TEST APPLIED.'
    ...
    Write-Host 'Switch one step at a time. NVIDIA is not considered ready until DLSS/DLAA reconstruction itself is active.'
    Write-Host 'Test ONLY: startup Off -> NVIDIA -> Off -> NVIDIA -> Off. Do NOT select AMD in P9B.2.'
