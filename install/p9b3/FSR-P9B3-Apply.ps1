...
$Candidate=Join-Path $PackageDir 'dlss5-feed-fsr-p9b3.addon64'
...
$State=Join-Path $Trex '_FSR_P9B3_LIVE_NVIDIA_OFF_TEST_STATE'
if(-not(Test-Path $Candidate)){throw "P9B3 candidate missing: $Candidate"}
foreach($p in @($Live,$Ini)){if(-not(Test-Path $p)){throw "Current install file missing: $p"}}
if(Test-Path $State){throw "A saved P9B3 state already exists at $State. Run 2-RESTORE-BEFORE-P9B3.bat first."}
...
try{
    ...
    Write-Host ''
    Write-Host 'P9B.3 LIVE OFF <-> NVIDIA TEST APPLIED.'
    Write-Host 'Initial backend: Off. NVIDIA switch must arm real SRProof and restore Quality, NR off.'
    ...
    Write-Host 'Switch one step at a time. P9B.3 also verifies a NVIDIA->Off request cannot be consumed by the NVIDIA SR finalizer.'
    Write-Host 'Test ONLY: startup Off -> NVIDIA -> Off -> NVIDIA -> Off. Do NOT select AMD in P9B.3.'
