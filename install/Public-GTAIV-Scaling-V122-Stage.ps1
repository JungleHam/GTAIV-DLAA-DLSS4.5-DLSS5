...
function Once([string]$Text,[string]$Old,[string]$New,[string]$Label){
    ...
    if($count-ne 1){throw "v1.2.2 stage $Label expected one match, found $count"}
...
$feed=Once $feed 'extern "C" __declspec(dllexport) const char *NAME = "DLSS 5 Feed " FEED_VERSION;' 'extern "C" __declspec(dllexport) const char *NAME = "GTA IV Scaling 1.2.2";' 'add-on display name'
...
foreach($marker in @(
    ...
    if(($vk+$feed).IndexOf($marker,[StringComparison]::Ordinal)-lt 0){throw "v1.2.2 marker missing: $marker"}
}
...
Write-Host 'v1.2.2 GTA IV Scaling identity + installer-driven RTX capability filtering applied.'
