...
function Once([string]$Text,[string]$Old,[string]$New,[string]$Label){
    ...
    if($count-ne 1){throw "FSR P9B3 stage \${Label}: expected one match, found $count"}
    return $Text.Replace($Old,$New)
}
function Replace-Function([string]$Text,[string]$Signature,[string]$New,[string]$Label){
    $start=$Text.IndexOf($Signature,[StringComparison]::Ordinal)
    if($start-lt 0){throw "FSR P9B3 stage \${Label}: function signature missing"}
    $brace=$Text.IndexOf('{',$start)
    if($brace-lt 0){throw "FSR P9B3 stage \${Label}: opening brace missing"}
    $depth=0;$end=-1
    ...
    if($end-lt 0){throw "FSR P9B3 stage \${Label}: closing brace missing"}
...
static void M3kSetSessionBackendSelection(UINT tech)
{
    ...
    Log("M3K-P9B3: session backend latch committed=%u",tech);
}
...
static bool M3kBackendNativeSourceReady()
{
    ...
    // P9B3: SRProfileApplied is only authoritative when the separate SR path is armed.
...
static void M3kRequestMasterEnabledLive(bool enabled)
{
    if (!enabled)
    {
        ...
        if(g_m3kScalingTechnologyActive==1u)
        {
            ...
            Log("M3K-P9B3: NVIDIA drain started through the proven DLAA Native path; saved NVIDIA profile=%s NR=%s",
                M3kSrProfileName(g_m3kMasterSavedProfile),g_m3kMasterSavedNr?"ON":"OFF");
        }
        else
        {
            Log("M3K-P9B3: %s drain started without modifying saved NVIDIA profile/NR state",
    ...
    if(g_m3kScalingTechnologyActive==1u)
    {
        ...
        Log("M3K-P9B3: NVIDIA enabled%s; restored profile=%s NR=%s",
    ...
    {
        Log("M3K-P9B3: %s enabled%s; NVIDIA profile/NR state left untouched",
            M3kScalingTechnologyName(g_m3kScalingTechnologyActive),
            wasPending?" after native drain":"");
    }
    g_m3k.ResetHistory();
...
static void M3kRequestScalingTechnologyLive(UINT tech)
{
    ...
    // P9B3 isolates only the new Off <-> NVIDIA lifecycle boundary.
    ...
    if(tech==2u || g_m3kScalingTechnologyActive==2u)
    {
        ...
        Log("M3K-P9B3: AMD live boundary is disabled in P9B3; active remains %s",
            M3kScalingTechnologyName(g_m3kScalingTechnologyActive));
        return;
    }
    ...
    if(tech==g_m3kScalingTechnologyActive)
    {
        if(g_m3kScalingTransitionPending)
        {
            ...
            Log("M3K-P9B3: backend switch cancelled; remaining on %s",
                M3kScalingTechnologyName(g_m3kScalingTechnologyActive));
        }
        return;
    }
    ...
    Log("M3K-P9B3: live switch %s %s -> %s; draining through native/raw midpoint",
...
static bool M3kCommitScalingTechnologyTransition()
{
    ...
    Log("M3K-P9B3: COMMIT begin %s -> %s at native %ux%u; old session teardown follows queue-idle safety",
    ...
    Log("M3K-P9B3: SRProof=%u for target %s",
    ...
    if(newTech==0u)
    {
        ...
        Log("M3K-P9B3: LIVE switch COMMITTED %s -> Off (native raw, sessionless)",
    ...
    else
    {
        ...
        M3kRequestMasterEnabledLive(true);
        Log("M3K-P9B3: target=%s selected; replacement session open is now REQUIRED before commit completes",
...
$initNew=@'
            ...
            Log("M3K-P9B3: active scaling technology captured at launch: %s",M3kScalingTechnologyName(m3kTech));
'@
...
$offInsertNew=@'
        ...
        if(g_m3kScalingTechnologyActive==0u&&!g_m3kScalingOffTransitionIssued&&M3kMasterEnabledRequested()){
            g_m3kScalingOffTransitionIssued=true;
            M3kRequestMasterEnabledLive(false);
            Log("M3K-P9B3: Off backend at launch; staged native raw transition started without opening NGX/FSR");
        }
        const UINT requested = g_m3kMasterEnabled ? GetPrivateProfileIntW(L"M3K", L"Mode", 0, path) : 0;
'@
...
$newBanner='        Log("M3K-FSR-P9B3: FSR 3.1.4 Vulkan context READY max=%ux%u depthInverted=%d RCAS=per-dispatch autoExposure=ON",'
...
static void M3kP9SessionOpenSucceeded()
{
    ...
    if(newTech==1u)
    {
        Log("M3K-P9B3: NVIDIA session READY after %s; waiting for requested reconstruction to become active",
            M3kScalingTechnologyName(oldTech));
        return;
    }
    ...
    Log("M3K-P9B3: LIVE switch COMMITTED %s -> %s; replacement session READY",
...
static void M3kP9TryFinalizeNvidiaTransition()
{
    ...
    {
        Log("M3K-P9B3: NVIDIA reconstruction FAILED after session open; rolling back through native to Off");
    ...
    g_m3kScalingTransitionLastFailed=false;
    M3kInvalidateBackendResolutionPlan();
    Log("M3K-P9B3: LIVE switch FINALIZED %s -> NVIDIA - DLAA / DLSS 4.5; reconstruction=%s READY source=%ux%u -> %ux%u",
...
static UINT M3kP9B3eginRollbackAfterTargetOpenFailure()
{
    ...
    Log("M3K-P9B3: target session %s FAILED to open; rolling back to %s",
...
static void M3kP9RollbackSucceeded(UINT oldTech,UINT failedTech)
{
    ...
    M3kInvalidateBackendResolutionPlan();
    Log("M3K-P9B3: ROLLBACK COMMITTED; %s restored after failed %s request",
...
static void M3kP9ForceOffAfterRollbackFailure(UINT failedTech,UINT oldTech)
{
    ...
    M3kWriteMasterIni(L"TemporalJitter",0);
    M3kInvalidateBackendResolutionPlan();
    Log("M3K-P9B3: ROLLBACK FAILED (%s after failed %s); forcing explicit Off/raw containment",
        M3kScalingTechnologyName(oldTech),M3kScalingTechnologyName(failedTech));
}
...
$sessionOpenNew=@'
    if (!g.session_ready)
    {
        ...
        if(g_m3kScalingTransitionAwaitingOpen)
        {
            ...
            {
                const UINT oldTech=M3kP9B3eginRollbackAfterTargetOpenFailure();
                if(oldTech==0u)
                {
...
$frameInsert=@'
    ...
    {
        // P9B3: sessionless Off must still run the backend-neutral public-state and
        ...
        if(g.width!=w||g.height!=h)
        {
            ...
            g_m3kResolutionPlanOutW=g_m3kResolutionPlanOutH=0;
            g_m3kResolutionConfirmedLogged=false;
            Log("M3K-P9B3: sessionless Off bootstrap output=%ux%u; initializing public state/native render plan",w,h);
        }
        ...
        if(M3kSessionBackendSelectionAtOpen()==0u)
        {
            ...
            if(M3kMasterNativePassthroughReady()&&!m3kOffReported){
                m3kOffReported=true;
                Log("M3K-P9B3: Off backend READY - true native/raw passthrough; no D3D12/NGX/FSR session is open");
            }
            return;
        }
    }
...
foreach($marker in @(
    'M3K-P9B3: COMMIT begin',
    'M3K-P9B3: LIVE switch COMMITTED',
    'M3K-P9B3: ROLLBACK COMMITTED',
    'M3K-P9B3: ROLLBACK FAILED',
    'M3K-P9B3: Off backend READY - true native/raw passthrough',
    ...
    'AMD live boundary is disabled in P9B3',
...
Write-Host 'FSR-P9B3 applied: NVIDIA finalization is target-gated so NVIDIA->Off cannot be stolen by the SR-ready path; AMD live crossing disabled.'
