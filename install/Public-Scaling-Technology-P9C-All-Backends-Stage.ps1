...
function Once([string]$Text,[string]$Old,[string]$New,[string]$Label){
    ...
    if($count-ne 1){throw "FSR P9C stage \${Label}: expected one match, found $count"}
    return $Text.Replace($Old,$New)
}
function Replace-Function([string]$Text,[string]$Signature,[string]$New,[string]$Label){
    $start=$Text.IndexOf($Signature,[StringComparison]::Ordinal)
    if($start-lt 0){throw "FSR P9C stage \${Label}: function signature missing"}
    $brace=$Text.IndexOf('{',$start)
    if($brace-lt 0){throw "FSR P9C stage \${Label}: opening brace missing"}
    $depth=0;$end=-1
    ...
    if($end-lt 0){throw "FSR P9C stage \${Label}: closing brace missing"}
...
static void M3kSetSessionBackendSelection(UINT tech)
{
    ...
    Log("M3K-P9C: session backend latch committed=%u",tech);
}
...
static UINT g_m3kScalingVendorHopFinalTarget=0xFFFFFFFFu;
static bool g_m3kScalingOffTransitionIssued=false;
...
static bool M3kBackendNativeSourceReady()
{
    ...
    // P9C: SRProfileApplied is only authoritative when the separate SR path is armed.
...
static void M3kRequestMasterEnabledLive(bool enabled)
{
    if (!enabled)
    {
        ...
        if(g_m3kScalingTechnologyActive==1u)
        {
            ...
            Log("M3K-P9C: NVIDIA drain started through the proven DLAA Native path; saved NVIDIA profile=%s NR=%s",
                M3kSrProfileName(g_m3kMasterSavedProfile),g_m3kMasterSavedNr?"ON":"OFF");
        }
        else
        {
            Log("M3K-P9C: %s drain started without modifying saved NVIDIA profile/NR state",
    ...
    if(g_m3kScalingTechnologyActive==1u)
    {
        ...
        Log("M3K-P9C: NVIDIA enabled%s; restored profile=%s NR=%s",
    ...
    {
        Log("M3K-P9C: %s enabled%s; NVIDIA profile/NR state left untouched",
            M3kScalingTechnologyName(g_m3kScalingTechnologyActive),
            wasPending?" after native drain":"");
    }
    g_m3k.ResetHistory();
...
static void M3kRequestScalingTechnologyLive(UINT tech)
{
    ...
    if(tech==g_m3kScalingTechnologyActive)
    {
        if(g_m3kScalingTransitionPending)
        {
            ...
            Log("M3K-P9C: backend switch cancelled; remaining on %s",
                M3kScalingTechnologyName(g_m3kScalingTechnologyActive));
        }
        return;
    }
    ...
    const bool vendorHop=
        g_m3kScalingTechnologyActive!=0u&&tech!=0u&&
        g_m3kScalingTechnologyActive!=tech;

    g_m3kScalingTransitionPending=true;
    g_m3kScalingVendorHopFinalTarget=vendorHop?tech:0xFFFFFFFFu;
    g_m3kScalingTransitionTarget=vendorHop?0u:tech;
    ...
    if(vendorHop)
        Log("M3K-P9C: live vendor switch requested %s -> %s; serializing through explicit Off/raw midpoint",
            M3kScalingTechnologyName(g_m3kScalingTechnologyActive),
            M3kScalingTechnologyName(tech));
    else
        Log("M3K-P9C: live switch %s %s -> %s; draining through native/raw midpoint",
...
$initNew=@'
            ...
            Log("M3K-P9C: active scaling technology captured at launch: %s",M3kScalingTechnologyName(m3kTech));
'@
...
$offInsertNew=@'
        ...
        if(g_m3kScalingTechnologyActive==0u&&!g_m3kScalingOffTransitionIssued&&M3kMasterEnabledRequested()){
            g_m3kScalingOffTransitionIssued=true;
            M3kRequestMasterEnabledLive(false);
            Log("M3K-P9C: Off backend at launch; staged native raw transition started without opening NGX/FSR");
        }
        const UINT requested = g_m3kMasterEnabled ? GetPrivateProfileIntW(L"M3K", L"Mode", 0, path) : 0;
'@
...
$newBanner='        Log("M3K-FSR-P9C: FSR 3.1.4 Vulkan context READY max=%ux%u depthInverted=%d RCAS=per-dispatch autoExposure=ON",'
...
static void M3kP9SessionOpenSucceeded()
{
    ...
    if(newTech==1u)
    {
        Log("M3K-P9C: NVIDIA session READY after %s; waiting for requested reconstruction to become active",
            M3kScalingTechnologyName(oldTech));
        return;
    }
    if(newTech==2u)
    {
        Log("M3K-P9C: AMD native Vulkan session READY after %s; waiting for first successful FSR dispatch",
            M3kScalingTechnologyName(oldTech));
        return;
    }
    ...
    Log("M3K-P9C: LIVE switch COMMITTED %s -> %s; replacement session READY",
        M3kScalingTechnologyName(oldTech),M3kScalingTechnologyName(newTech));
}

static bool M3kP9NvidiaReconstructionReady()
{
    if(!g.session_ready||g.feature==nullptr||g_m3kScalingTechnologyActive!=1u)return false;
    const UINT profile=M3kRequestedSrProfile();
    if(profile==0u)
        return !g_m3kSrFeatureActive&&g_m3kSrProfileApplied==0u&&
               g_m3kSourceTapReady&&
               g_m3kPresentSource.width==g.width&&g_m3kPresentSource.height==g.height;
    return g_m3kSrFeatureActive&&g_m3kSrProfileApplied==profile&&
           g_m3kSourceTapReady&&g_m3kDesiredRenderW&&g_m3kDesiredRenderH&&
           g_m3kPresentSource.width==g_m3kDesiredRenderW&&
...
static void M3kP9TryFinalizeNvidiaTransition()
{
    ...
    if(g_m3kScalingTransitionTarget!=1u)
    {
        if(!m3kNonNvidiaTargetGuardReported)
        {
            ...
            Log("M3K-P9C: NVIDIA reconstruction finalizer BLOCKED for pending target %s; non-NVIDIA transition remains authoritative",
    ...
    {
        Log("M3K-P9C: NVIDIA reconstruction FAILED after session open; rolling back through native to Off");
        M3kRequestScalingTechnologyLive(0u);
        g_m3kScalingTransitionLastFailed=true;
        return;
    }

    if(!M3kP9NvidiaReconstructionReady())return;

    const UINT oldTech=g_m3kScalingTransitionPrevious;
    const UINT profile=M3kRequestedSrProfile();
    g_m3kScalingTransitionPending=false;
    g_m3kScalingTransitionPrevious=1u;
    g_m3kScalingTransitionNativeOverride=false;
    g_m3kScalingTransitionLastFailed=false;
    M3kInvalidateBackendResolutionPlan();
    Log("M3K-P9C: LIVE switch FINALIZED %s -> NVIDIA - DLAA / DLSS 4.5; reconstruction=%s READY source=%ux%u -> %ux%u",
        M3kScalingTechnologyName(oldTech),M3kSrProfileName(profile),
        g_m3kPresentSource.width,g_m3kPresentSource.height,g.width,g.height);
}

static UINT M3kP9CBeginRollbackAfterTargetOpenFailure()
{
    ...
    Log("M3K-P9C: target session %s FAILED to open; rolling back to %s",
...
static void M3kP9RollbackSucceeded(UINT oldTech,UINT failedTech)
{
    ...
    M3kInvalidateBackendResolutionPlan();
    Log("M3K-P9C: ROLLBACK COMMITTED; %s restored after failed %s request",
...
static void M3kP9ForceOffAfterRollbackFailure(UINT failedTech,UINT oldTech)
{
    ...
    M3kWriteMasterIni(L"TemporalJitter",0);
    M3kInvalidateBackendResolutionPlan();
    Log("M3K-P9C: ROLLBACK FAILED (%s after failed %s); forcing explicit Off/raw containment",
        M3kScalingTechnologyName(oldTech),M3kScalingTechnologyName(failedTech));
}
...
$sessionOpenNew=@'
    if (!g.session_ready)
    {
        ...
        if(g_m3kScalingTransitionAwaitingOpen)
        {
            ...
            {
                const UINT oldTech=M3kP9CBeginRollbackAfterTargetOpenFailure();
                if(oldTech==0u)
                {
...
$frameInsert=@'
    ...
    {
        // P9C: sessionless Off must still run the backend-neutral public-state and
        ...
        if(g.width!=w||g.height!=h)
        {
            ...
            g_m3kResolutionPlanOutW=g_m3kResolutionPlanOutH=0;
            g_m3kResolutionConfirmedLogged=false;
            Log("M3K-P9C: sessionless Off bootstrap output=%ux%u; initializing public state/native render plan",w,h);
        }
        ...
        if(M3kSessionBackendSelectionAtOpen()==0u)
        {
            ...
            if(M3kMasterNativePassthroughReady()&&!m3kOffReported){
                m3kOffReported=true;
                Log("M3K-P9C: Off backend READY - true native/raw passthrough; %s",
                    g_m3kFsrNativeSession?"dormant FSR Vulkan session retained":"sessionless");
            }
            return;
        }
...
$uiPendingNew=@'
        if(M3kScalingTechnologyTransitionPending())
            ImGui::TextColored(ImVec4(1.0f,0.78f,0.25f,1.0f),
                "Switching to %s - backend/reconstruction transition in progress...",
...
$fsrDispatchNew=@'
    if(!g_m3kFsrBackend.Dispatch(d)){
        ...
        Log("M3K-FSR-P1: dispatch FAILED; proof latched failed; NGX fallback forbidden");
        if(g_m3kScalingTransitionPending&&!g_m3kScalingTransitionAwaitingOpen&&
           g_m3kScalingTechnologyActive==2u&&g_m3kScalingTransitionTarget==2u)
        {
            Log("M3K-P9C: AMD reconstruction FAILED after session ready; containing explicitly to Off/raw");
            M3kRequestScalingTechnologyLive(0u);
            g_m3kScalingTransitionLastFailed=true;
        }
        return false;
       ...
       g_m3kScalingTechnologyActive==2u&&g_m3kScalingTransitionTarget==2u)
    {
        const UINT oldTech=g_m3kScalingTransitionPrevious;
        g_m3kScalingTransitionPending=false;
        g_m3kScalingTransitionPrevious=2u;
        g_m3kScalingTransitionNativeOverride=false;
        g_m3kScalingTransitionLastFailed=false;
        g_m3kScalingVendorHopFinalTarget=0xFFFFFFFFu;
        Log("M3K-P9C: LIVE switch FINALIZED %s -> AMD - FSR; reconstruction READY source=%ux%u -> %ux%u scale=%.1f%%",
            M3kScalingTechnologyName(oldTech),info.width,info.height,finalW,finalH,
            static_cast<double>(g_m3kFsrScalePermille)/10.0);
    }
'@
$vk=Once $vk $fsrDispatchOld $fsrDispatchNew 'AMD finalizes only after real FSR dispatch'
...
foreach($marker in @(
    ...
    'live vendor switch requested %s -> %s; serializing through explicit Off/raw midpoint',
    'VENDOR MIDPOINT COMMITTED %s -> Off/raw; continuing Off -> %s on the next frame',
    'AMD native Vulkan session READY after %s; waiting for first successful FSR dispatch',
    'LIVE switch FINALIZED %s -> AMD - FSR; reconstruction READY',
    'AMD -> Off retains the proven dormant native Vulkan FSR session',
    'Off -> AMD reuses the retained native Vulkan FSR session'
)){
...
Write-Host 'FSR-P9C applied: full Off/NVIDIA/AMD live composition with vendor switches serialized through Off/raw.'
