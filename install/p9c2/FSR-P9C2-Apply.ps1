...
$Candidate=Join-Path $PackageDir 'dlss5-feed-fsr-p9c2.addon64'
...
$State=Join-Path $Trex '_FSR_P9C2_ALL_BACKENDS_TEST_STATE'
if(-not(Test-Path $Candidate)){throw "P9C2 candidate missing: $Candidate"}
foreach($p in @($Live,$Ini)){if(-not(Test-Path $p)){throw "Current install file missing: $p"}}
if(Test-Path $State){throw "A saved P9C2 state already exists at $State. Run 2-RESTORE-BEFORE-P9C2.bat first."}
...
try{
    ...
    Set-Ini $Ini 'M3K' 'FSRScalePermille' '100'
    ...
    Write-Host ''
    Write-Host 'P9C.2 FSR FIXED-EXPOSURE TEST APPLIED.'
    Write-Host 'Initial backend: Off. NVIDIA=Quality; AMD=10% + RCAS OFF.'
    ...
    Write-Host 'This test changes FSR exposure only; backend switching remains the P9C/P9C1 pipeline.'
    Write-Host 'Focus: Off -> AMD first. Judge the blinking at 10% (known temporal state), then 66.7% and 100%.'
