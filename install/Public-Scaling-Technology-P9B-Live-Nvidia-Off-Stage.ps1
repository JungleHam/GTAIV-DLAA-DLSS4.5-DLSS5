...
function Once([string]$Text,[string]$Old,[string]$New,[string]$Label){
    ...
    if($count-ne 1){throw "FSR P9B stage \${Label}: expected one match, found $count"}
    return $Text.Replace($Old,$New)
}
function Replace-Function([string]$Text,[string]$Signature,[string]$New,[string]$Label){
    $start=$Text.IndexOf($Signature,[StringComparison]::Ordinal)
    if($start-lt 0){throw "FSR P9B stage \${Label}: function signature missing"}
    $brace=$Text.IndexOf('{',$start)
    if($brace-lt 0){throw "FSR P9B stage \${Label}: opening brace missing"}
    $depth=0;$end=-1
    ...
    if($end-lt 0){throw "FSR P9B stage \${Label}: closing brace missing"}
...
static void M3kSetSessionBackendSelection(UINT tech)
{
    ...
    Log("M3K-P9B: session backend latch committed=%u",tech);
}
...
static void M3kRequestMasterEnabledLive(bool enabled)
{
    if (!enabled)
    {
        ...
        if(g_m3kScalingTechnologyActive==1u)
        {
            ...
            Log("M3K-P9B: NVIDIA drain started through the proven DLAA Native path; saved NVIDIA profile=%s NR=%s",
                M3kSrProfileName(g_m3kMasterSavedProfile),g_m3kMasterSavedNr?"ON":"OFF");
        }
        else
        {
            Log("M3K-P9B: %s drain started without modifying saved NVIDIA profile/NR state",
    ...
    if(g_m3kScalingTechnologyActive==1u)
    {
        ...
        Log("M3K-P9B: NVIDIA enabled%s; restored profile=%s NR=%s",
    ...
    {
        Log("M3K-P9B: %s enabled%s; NVIDIA profile/NR state left untouched",
            M3kScalingTechnologyName(g_m3kScalingTechnologyActive),
            wasPending?" after native drain":"");
    }
    g_m3k.ResetHistory();
...
static void M3kRequestScalingTechnologyLive(UINT tech)
{
    ...
    // P9B isolates only the new Off <-> NVIDIA lifecycle boundary.
    ...
    if(tech==2u || g_m3kScalingTechnologyActive==2u)
    {
        ...
        Log("M3K-P9B: AMD live boundary is disabled in P9B; active remains %s",
            M3kScalingTechnologyName(g_m3kScalingTechnologyActive));
        return;
    }
    ...
    if(tech==g_m3kScalingTechnologyActive)
    {
        if(g_m3kScalingTransitionPending)
        {
            ...
            Log("M3K-P9B: backend switch cancelled; remaining on %s",
                M3kScalingTechnologyName(g_m3kScalingTechnologyActive));
        }
        return;
    }
    ...
    Log("M3K-P9B: live switch %s %s -> %s; draining through native/raw midpoint",
...
static bool M3kCommitScalingTechnologyTransition()
{
    ...
    Log("M3K-P9B: COMMIT begin %s -> %s at native %ux%u; old session teardown follows queue-idle safety",
    ...
    if(newTech==0u)
    {
        ...
        Log("M3K-P9B: LIVE switch COMMITTED %s -> Off (native raw, sessionless)",
    ...
    else
    {
        ...
        M3kRequestMasterEnabledLive(true);
        Log("M3K-P9B: target=%s selected; replacement session open is now REQUIRED before commit completes",
...
$initNew=@'
            ...
            Log("M3K-P9B: active scaling technology captured at launch: %s",M3kScalingTechnologyName(m3kTech));
'@
...
$offInsertNew=@'
        ...
        if(g_m3kScalingTechnologyActive==0u&&!g_m3kScalingOffTransitionIssued&&M3kMasterEnabledRequested()){
            g_m3kScalingOffTransitionIssued=true;
            M3kRequestMasterEnabledLive(false);
            Log("M3K-P9B: Off backend at launch; staged native raw transition started without opening NGX/FSR");
        }
        const UINT requested = g_m3kMasterEnabled ? GetPrivateProfileIntW(L"M3K", L"Mode", 0, path) : 0;
'@
...
$newBanner='        Log("M3K-FSR-P9B: FSR 3.1.4 Vulkan context READY max=%ux%u depthInverted=%d RCAS=per-dispatch autoExposure=ON",'
...
static void M3kP9SessionOpenSucceeded()
{
    ...
    M3kInvalidateBackendResolutionPlan();
    Log("M3K-P9B: LIVE switch COMMITTED %s -> %s; replacement session READY",
...
static UINT M3kP9BeginRollbackAfterTargetOpenFailure()
{
    ...
    Log("M3K-P9B: target session %s FAILED to open; rolling back to %s",
...
static void M3kP9RollbackSucceeded(UINT oldTech,UINT failedTech)
{
    ...
    M3kInvalidateBackendResolutionPlan();
    Log("M3K-P9B: ROLLBACK COMMITTED; %s restored after failed %s request",
...
static void M3kP9ForceOffAfterRollbackFailure(UINT failedTech,UINT oldTech)
{
    ...
    M3kWriteMasterIni(L"TemporalJitter",0);
    M3kInvalidateBackendResolutionPlan();
    Log("M3K-P9B: ROLLBACK FAILED (%s after failed %s); forcing explicit Off/raw containment",
        M3kScalingTechnologyName(oldTech),M3kScalingTechnologyName(failedTech));
}
...
$frameInsert=@'
    ...
    if(m3kBackendAtFrame==0u)
    {
        ...
        if(!m3kOffReported){
            m3kOffReported=true;
            Log("M3K-P9B: Off backend active - Vulkan frame passes untouched; no D3D12/NGX/FSR session is opened");
        }
        return;
    }
...
foreach($marker in @(
    'M3K-P9B: COMMIT begin',
    'M3K-P9B: LIVE switch COMMITTED',
    'M3K-P9B: ROLLBACK COMMITTED',
    'M3K-P9B: ROLLBACK FAILED',
    'M3K-P9B: Off backend active - Vulkan frame passes untouched',
    ...
    'AMD live boundary is disabled in P9B'
)){
...
Write-Host 'FSR-P9B applied: isolated live Off<->NVIDIA switching; AMD live crossing disabled in this candidate.'
