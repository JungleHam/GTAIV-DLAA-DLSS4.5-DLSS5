...
$Candidate=Join-Path $PackageDir 'dlss5-feed-fsr-p9c5.addon64'
...
$State=Join-Path $Trex '_FSR_P9C5_ALL_BACKENDS_TEST_STATE'
if(-not(Test-Path $Candidate)){throw "P9C5 candidate missing: $Candidate"}
foreach($p in @($Live,$Ini)){if(-not(Test-Path $p)){throw "Current install file missing: $p"}}
if(Test-Path $State){throw "A saved P9C5 state already exists at $State. Run 2-RESTORE-BEFORE-P9C5.bat first."}
...
try{
    ...
    Set-Ini $Ini 'M3K' 'FSRSharpness' '0.000'
    ...
    Set-Ini $Ini 'M3K' 'JitterMode' '8'
    Set-Ini $Ini 'M3K' 'NvidiaJitterCompMode' '1'
    ...
    Write-Host ''
    Write-Host 'P9C.5 CALIBRATED NVIDIA AA DEFAULTS TEST APPLIED.'
    Write-Host 'Initial backend: Off. NVIDIA=Quality with FULL raster jitter; AMD=66.7% + RCAS OFF.'
    ...
    Write-Host 'This test preserves P9C.2 fixed-exposure FSR and full raster jitter. NVIDIA now starts at the hardware-calibrated 8 phases + -X/-Y. Advanced controls remain available.'
    Write-Host 'Focus: Off -> NVIDIA Quality and confirm the UI already shows 8 phases and -X/-Y; then smoke-test DLAA and AMD.'
