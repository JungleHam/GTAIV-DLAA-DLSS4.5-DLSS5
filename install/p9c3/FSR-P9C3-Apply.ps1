...
$Candidate=Join-Path $PackageDir 'dlss5-feed-fsr-p9c3.addon64'
...
$State=Join-Path $Trex '_FSR_P9C3_ALL_BACKENDS_TEST_STATE'
if(-not(Test-Path $Candidate)){throw "P9C3 candidate missing: $Candidate"}
foreach($p in @($Live,$Ini)){if(-not(Test-Path $p)){throw "Current install file missing: $p"}}
if(Test-Path $State){throw "A saved P9C3 state already exists at $State. Run 2-RESTORE-BEFORE-P9C3.bat first."}
...
try{
    ...
    Set-Ini $Ini 'M3K' 'TemporalJitter' '0'
    ...
    Write-Host ''
    Write-Host 'P9C.3 NVIDIA UNJITTERED-RASTER TEST APPLIED.'
    Write-Host 'Initial backend: Off. NVIDIA=Quality with raster jitter OFF; AMD=66.7% + RCAS OFF.'
    ...
    Write-Host 'This test changes only backend jitter policy: NVIDIA OFF, AMD ON. P9C.2 fixed FSR exposure is preserved.'
    Write-Host 'Focus: Off -> NVIDIA first. Judge shimmer while panning, then verify AMD remains clean.'
