...
if(-not(Test-Path $State)){throw "No saved P9C3 state exists at $State"}

if(Test-Path $Log){Copy-Item $Log (Join-Path $PackageDir 'FSR-P9C3-TEST-LOG.txt') -Force}
...
Write-Host ''
Write-Host 'PRE-P9C3 STATE RESTORED.'
if(Test-Path (Join-Path $PackageDir 'FSR-P9C3-TEST-LOG.txt')){Write-Host 'Send me FSR-P9C3-TEST-LOG.txt from this folder.'}
