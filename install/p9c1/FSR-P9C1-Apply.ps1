...
$Candidate=Join-Path $PackageDir 'dlss5-feed-fsr-p9c1.addon64'
...
$State=Join-Path $Trex '_FSR_P9C1_ALL_BACKENDS_TEST_STATE'
if(-not(Test-Path $Candidate)){throw "P9C1 candidate missing: $Candidate"}
foreach($p in @($Live,$Ini)){if(-not(Test-Path $p)){throw "Current install file missing: $p"}}
if(Test-Path $State){throw "A saved P9C1 state already exists at $State. Run 2-RESTORE-BEFORE-P9C1.bat first."}
...
try{
    ...
    Write-Host ''
    Write-Host 'P9C.1 AMD JITTER RE-ARM TEST APPLIED.'
    Write-Host 'Initial backend: Off. NVIDIA=Quality; AMD=77% + RCAS 0.25.'
    ...
    Write-Host 'Switch one step at a time. P9C1 also verifies a NVIDIA->Off request cannot be consumed by the NVIDIA SR finalizer.'
    Write-Host 'Focus: NVIDIA -> AMD direct at 77%, then AMD 100% and 10%. Watch P9C1 jitter re-arm diagnostics.'
