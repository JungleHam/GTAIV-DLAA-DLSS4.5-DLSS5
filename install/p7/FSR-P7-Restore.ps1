...
if(-not(Test-Path $State)){throw "No saved P7 state exists at $State"}
if(Test-Path $Log){Copy-Item $Log (Join-Path $PackageDir 'FSR-P7-TEST-LOG.txt') -Force}
...
Write-Host ''
Write-Host 'PRE-P7 STATE RESTORED.'
if(Test-Path (Join-Path $PackageDir 'FSR-P7-TEST-LOG.txt')){Write-Host 'Send me FSR-P7-TEST-LOG.txt if you ran the game.'}
