...
$Candidate=Join-Path $PackageDir 'dlss5-feed-fsr-p7.addon64'
...
$State=Join-Path $Trex '_FSR_P7_SELECTOR_TEST_STATE'
if(-not(Test-Path $Candidate)){throw "P7 candidate missing: $Candidate"}
foreach($p in @($Live,$Ini)){if(-not(Test-Path $p)){throw "Current install file missing: $p"}}
if(Test-Path $State){throw "A saved P7 state already exists at $State. Run 2-RESTORE-BEFORE-P7.bat first."}
...
try{
    ...
    Write-Host ''
    Write-Host 'P7 SCALING TECHNOLOGY SELECTOR TEST APPLIED.'
    Write-Host 'Initial backend: AMD - FSR.'
    Write-Host 'd3d9.dll and NvRemixBridge.exe are untouched.'
    Write-Host ''
    Write-Host 'Launch GTA IV and inspect Home -> Add-ons -> DLSS 5 Feed -> GTA IV Reconstruction.'
    Write-Host 'The technology selector should show AMD - FSR and only AMD controls.'
    Write-Host 'Changing Technology is intentionally restart-gated in P7.'
    Write-Host 'Close GTA IV and run 2-RESTORE-BEFORE-P7.bat when finished.'
}catch{
    try{
