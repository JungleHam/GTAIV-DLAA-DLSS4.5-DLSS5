...
if(-not(Test-Path $State)){throw "No saved P9BB state exists at $State"}

if(Test-Path $Log){Copy-Item $Log (Join-Path $PackageDir 'FSR-P9B-TEST-LOG.txt') -Force}
...
Write-Host ''
Write-Host 'PRE-P9B STATE RESTORED.'
if(Test-Path (Join-Path $PackageDir 'FSR-P9B-TEST-LOG.txt')){Write-Host 'Send me FSR-P9B-TEST-LOG.txt from this folder.'}
