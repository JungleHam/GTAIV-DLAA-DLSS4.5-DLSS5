...
$Candidate=Join-Path $PackageDir 'dlss5-feed-fsr-p9b.addon64'
...
$State=Join-Path $Trex '_FSR_P9B_LIVE_NVIDIA_OFF_TEST_STATE'
if(-not(Test-Path $Candidate)){throw "P9B candidate missing: $Candidate"}
foreach($p in @($Live,$Ini)){if(-not(Test-Path $p)){throw "Current install file missing: $p"}}
if(Test-Path $State){throw "A saved P9B state already exists at $State. Run 2-RESTORE-BEFORE-P9B.bat first."}
...
try{
    ...
    Write-Host ''
    Write-Host 'P9B LIVE OFF <-> NVIDIA TEST APPLIED.'
    ...
    Write-Host 'Test ONLY: Off -> NVIDIA -> Off -> NVIDIA -> Off. Do NOT select AMD in P9B.'
