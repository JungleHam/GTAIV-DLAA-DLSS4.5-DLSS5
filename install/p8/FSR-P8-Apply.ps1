...
$Candidate=Join-Path $PackageDir 'dlss5-feed-fsr-p8.addon64'
...
$State=Join-Path $Trex '_FSR_P8_RCAS_TEST_STATE'
if(-not(Test-Path $Candidate)){throw "P8 candidate missing: $Candidate"}
foreach($p in @($Live,$Ini)){if(-not(Test-Path $p)){throw "Current install file missing: $p"}}
if(Test-Path $State){throw "A saved P8 state already exists at $State. Run 2-RESTORE-BEFORE-P8.bat first."}
...
try{
    ...
    Write-Host ''
    Write-Host 'P8 AMD FSR RCAS TEST APPLIED.'
    Write-Host 'Backend: AMD - FSR; scale 66.7%; RCAS sharpness 0.25.'
    Write-Host 'd3d9.dll and NvRemixBridge.exe are untouched.'
    Write-Host ''
    Write-Host 'Launch GTA IV and open GTA IV Reconstruction. The AMD panel should show its own RCAS slider.'
    Write-Host 'Try 0.00, 0.25 and 0.50 while playing; changes are live and do not change NVIDIA sharpening.'
    Write-Host 'Close GTA IV and run 2-RESTORE-BEFORE-P8.bat.'
}catch{
    try{
