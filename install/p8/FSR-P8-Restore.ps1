...
if(-not(Test-Path $State)){throw "No saved P8 state exists at $State"}
if(Test-Path $Log){Copy-Item $Log (Join-Path $PackageDir 'FSR-P8-TEST-LOG.txt') -Force}
...
Write-Host ''
Write-Host 'PRE-P8 STATE RESTORED.'
if(Test-Path (Join-Path $PackageDir 'FSR-P8-TEST-LOG.txt')){Write-Host 'Send me FSR-P8-TEST-LOG.txt if you ran the game.'}
