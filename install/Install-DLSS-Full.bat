...
function Snapshot-Current([string]$Game,[string]$Dest){New-Item -ItemType Directory -Path $Dest -Force|Out-Null;foreach($rel in @('d3d9.dll','dxvk.conf','.trex\NvRemixBridge.exe','.trex\d3d9vk_x64.dll','.trex\dlss5-feed.addon64','.trex\dlss5-feed.cfg','.trex\m3k-nr.ini','.trex\m3k\m3k-nvngx.dll','.trex\m3k\nvngx_dlssnr.dll','DLSS-Full-Control.bat','Uninstall-DLSS-Full.bat','DLSS_FULL_INSTALLED.txt')){Copy-Path (Join-Path $Game $rel) (Join-Path $Dest $rel)}}
function Restore-Snapshot([string]$Game,[string]$Dest){foreach($rel in @('d3d9.dll','dxvk.conf','.trex\NvRemixBridge.exe','.trex\d3d9vk_x64.dll','.trex\dlss5-feed.addon64','.trex\dlss5-feed.cfg','.trex\m3k-nr.ini','.trex\m3k\m3k-nvngx.dll','.trex\m3k\nvngx_dlssnr.dll','DLSS-Full-Control.bat','Uninstall-DLSS-Full.bat','DLSS_FULL_INSTALLED.txt')){$dst=Join-Path $Game $rel;if(Test-Path -LiteralPath $dst){Remove-Item -LiteralPath $dst -Force -ErrorAction SilentlyContinue};Copy-Path (Join-Path $Dest $rel) $dst}}
...
try{
 ...
 $feedCfg=Join-Path $Trex 'dlss5-feed.cfg';Set-KeyEquals $feedCfg 'enabled' '1';Set-KeyEquals $feedCfg 'mode' '2';Set-KeyEquals $feedCfg 'work_resolution' '100';Set-KeyEquals (Join-Path $Game 'dxvk.conf') 'd3d9.samplerLodBias' '0.0'
